...
class VarRenamerBase(TransformationBase):
    ...
    def __init__(
        ...
        self.PROHIBITED_VARS = keyword.kwlist

    def extract_var_names(self, root, code_string, f_string=False):
        var_names = []
        queue = [root]
        idx_slices = []
        ...
        while len(queue) > 0:
            ...
            if (current_node.type == "identifier" or current_node.type == "variable_name") and str(
                ...
                if current_node.parent.type == "attribute":
                    children = current_node.parent.children
                    current_index = children.index(current_node)
                    if current_index > 0 and children[current_index - 1].type == ".":
                        pass
                    else:
                        var_names.append(self.tokenizer_function(code_string, current_node)[0])
                        if f_string:
                            idx_slices.append((current_node.start_point[1], current_node.end_point[1]))
                else:
                    var_names.append(self.tokenizer_function(code_string, current_node)[0])
                    if f_string:
                        idx_slices.append((current_node.start_point[1], current_node.end_point[1]))
    ...
    def get_not_var_ptype_var_names(self, root, code_string):
        ...
        while len(queue) > 0:
            ...
            queue = queue[1:]
            if (current_node.type == "identifier" or current_node.type == "variable_name") and str(
                    current_node.parent.type) in self.not_var_ptype:
                var_names.append(self.tokenizer_function(code_string, current_node)[0])
            for child in current_node.children:
                queue.append(child)
        return var_names
    ...
    def get_import_var_names(self, root, code_string):
        ...
        import_code_string = "\n".join(new_lines)
        import_var_names, _ = self.extract_var_names(self.parse_code(import_code_string), import_code_string)
        return list(set(import_var_names))

    def get_frequency_dict(self, var_names):
        frequency_dict = {}
        for var in var_names:
            if var in frequency_dict:
                frequency_dict[var] += 1
            else:
                frequency_dict[var] = 1
        return frequency_dict
    
    def select_most_frequent_var(self, var_names):
        # counts = {}
        # for var in var_names:
        #     counts[var] = original_code.count(var)
        max_cnt = -1
        max_var = None
        for var, cnt in var_names.items():
            if cnt > max_cnt:
                max_cnt = cnt
                max_var = var
        return max_var
    ...
    def is_f_string(self, t):
        return t.startswith(("f", "F")) and "{" in t and "}" in t
    ...
    def var_renaming(self, code_string, method, debug=False):
        ...
        var_names, _ = self.extract_var_names(root, code_string)
        # var_names = list(set(var_names))
        var_names = self.get_frequency_dict(var_names)
        ...
        selected_var = self.select_most_frequent_var(var_names)
        ...
        if selected_var is not None:
            ...
            if method == "CodeBERT":
                ...
                new_code_string = code_string.replace(selected_var, " <mask> ").replace(". <mask> ", f".{selected_var}")
            ...
            if replace_var is not None:
                ...
                if replace_var in self.TYPE_VARS or replace_var in self.PROHIBITED_VARS:
                    replace_var = "var_" + replace_var
                
                if replace_var in var_names:
                    num = 1
                    new_replace_var = replace_var + str(num)
                    while new_replace_var in var_names:
                        num += 1
                        new_replace_var = replace_var + str(num)
                    replace_var = new_replace_var
        ...
        for idx in range(len(original_code)):
            ...
            if selected_var is not None and replace_var is not None and t == selected_var:
                ...
                if original_code[idx - 1] != ".":
                    modified_code.append(replace_var)
                else:
                    modified_code.append(t)
            elif selected_var is not None and replace_var is not None and self.is_f_string(t):
                    f_string_vars, idx_slices = self.extract_var_names(
                        self.parse_code(t), t, f_string=True
                    )
                    assert len(f_string_vars) == len(idx_slices), "Unequal length!"
                    
                    diff = len(replace_var) - len(selected_var)
                    offset = 0
                    for i in range(len(f_string_vars)):
                        if f_string_vars[i] == selected_var:
                            start_idx, end_idx = idx_slices[i]
                            adj_start = start_idx + offset
                            adj_end = end_idx + offset
                            t = t[:adj_start] + replace_var + t[adj_end:]
                            offset += diff
                    modified_code.append(t)
            else:
                modified_code.append(t)
